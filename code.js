...
const songList = [
    ...
    ["The Life of a Showgirl", [232, 233 ,234 ,235 ,236 ,237 ,238 ,239 ,240 ,241 ,242 ,243]]
]
const NUMBER_OF_SONGS = 244;
...
const startGame = () => {
    ...
    window.addEventListener('pagehide', saveProgressInLocalStorage);
...
const toggleHidden = (e) => e.currentTarget.classList.toggle('hidden');
...
const finishGame = () => {
    ...
    window.removeEventListener('pagehide', saveProgressInLocalStorage);
...
const customAlert = (wrapper, goBackTo) => {
    ...
    let promise = new Promise((resolve, reject) => {
        wrapper.addEventListener('click', function onClickWrapper (event) {
            ...
            // buttons in settings and filter
...
const keepGameClick = (event) => {
    if (event.target.classList.contains('close-btn')) {
        ...
        isNewGame = false;
        minutesLeft = Number(localStorage.getItem("minutesLeft"));
        ...
        document.getElementById("keep-last-game").removeEventListener('click', keepGameClick);
        document.getElementById("keep-last-game").classList.add('none');
    }
...
// -------- start the program -----------------
// pre fetch new song
const songFetchPromise = fetchSong();


window.addEventListener("load", () => {
    // check for saved game
    const wrapper = document.getElementById("keep-last-game")
    if (localStorage.getItem("isGameOver") === "false") {
        wrapper.classList.remove('none');
        // add event listeners to popup
        wrapper.addEventListener('click', keepGameClick);
    } else {
